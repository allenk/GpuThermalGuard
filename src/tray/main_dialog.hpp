...
public:
    ...
    BEGIN_MSG_MAP(MainDialog)
        ...
        MESSAGE_HANDLER(OsdOverlay::kPlacementChangedMessage, OnOsdPlacementChanged)
...
private:
    ...
    LRESULT OnOsdPlacementChanged(UINT, WPARAM, LPARAM, BOOL&);
    ...
    void EvaluateProtection(const nvml::DeviceSnapshot& snapshot);
    ...
    std::unique_ptr<ProtectionController> controller_;
    ...
    std::int64_t last_safe_retry_ms_{0};
    std::int64_t last_auto_restore_attempt_ms_{0};
    std::uint64_t last_journal_snapshot_ms_{0};
    bool gpu_unavailable_logged_{false};
    std::optional<ProtectionState> last_logged_state_;
