...
| 主監控面板，七條歷史紀錄 | 展開 OSD，依自訂順序排列 |
| --- | --- |
| ![GpuThermalGuard 主面板：溫度、功率、VRAM、GPU、CPU、RAM 與 FPS 歷史](docs/images/dashboard-zh-TW.png) | ![展開 OSD 以自訂順序顯示七列數值](docs/images/osd-zh-TW.png) |

截圖中的設定屬於單一工作站，不是所有 GPU 的建議門檻。

### Compact 精簡 OSD

![精簡 OSD：一排七項即時數值與 30 秒迷你趨勢曲線](docs/images/osd-compact-zh-TW.png)
...
![同一個精簡 OSD 排成兩排，上排五張、下排兩張](docs/images/osd-compact-two-rows-zh-TW.png)
...
第一排至少保留五張，所以只有五個項目時一定是單排。展開 OSD 也依照同一個順序。排列會被記住；托盤選單的「重設 OSD 版面」可回到預設。
