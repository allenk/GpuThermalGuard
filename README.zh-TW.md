...
點擊 OSD 頂部的箭頭，可在完整圖表與 compact mode 之間切換。精簡模式以小卡顯示溫度、功率、VRAM、GPU 與 CPU 使用率，搭配即時數值及最近 30 秒的迷你曲線。勾選「顯示 RAM」或「顯示 FPS」各會增加一張卡；取消勾選後回到其餘項目的配置。
...
自 v0.11.0-beta.1 起，精簡面板預設是「啟用項目排成一排」，而且可以自行調整。箭頭左邊的鎖圖示可切換鎖定與解鎖：鎖定時小方塊不接收滑鼠，解鎖時才能拖動。

解鎖後，按住一個小方塊，放開在你要的位置即可。放在同一排就是調整順序；往下放會移到第二排，往上放則回到第一排。第一排至少保留五個，所以只有五個項目時一定是單排。排列會被記住；托盤選單的「重設 OSD 版面」可回到預設。

鎖只鎖小方塊。不論鎖定與否，OSD 本身仍可由頂部長條拖曳移動。
